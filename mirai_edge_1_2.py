...
def get_rsi(data, start_index, end_index, period):
    rsi = []
...
def get_bbs(data, start_index, end_index, period, standard_deviations):
    # ----
...
def mirai_no_data(ticker_data, start_date, end_date, STD_DEVIATIONS, SMA_PERIOD, RSI_PERIOD, RSI_LOW, RSI_HIGH, MAX_HOLD):
...
def mirai_no_yaiba(x, y, c, MIN_OPERATIONS):
    DETAIL_SLOPE = 20
    DETAIL_INTERCEPT = 20
    MAX_Y_FACTOR = 5.0
    MIN_PERCENTAGE = 0.85
...
def mirai_no_sentaku(batch_number, number_of_batches, start_date, end_date):
    # if len(sys.argv) > 1:
    ...
    for i in range(tickers_per_batch * (batch_number - 1), tickers_per_batch * batch_number):
        ...
        # EXPECTED MOVE
        EXPECTED_MOVE_MIN = 0.05
        EXPECTED_MOVE_MAX = 0.05
        EXPECTED_MOVE_ITERATIONS = 1
        # LOSS FACTOR
        LOSS_FACTOR_MIN = 2
        LOSS_FACTOR_MAX = 2
                                    ...
                                    for MAX_HOLD in range(MAX_HOLD_MIN, MAX_HOLD_MAX + MAX_HOLD_STEP, MAX_HOLD_STEP):
                                        for MIN_OPERATIONS_BUY in range(MIN_OPERATIONS_MIN_BUY, MIN_OPERATIONS_MAX_BUY + MIN_OPERATIONS_STEP, MIN_OPERATIONS_STEP):
                                            for MIN_OPERATIONS_SELL in range(MIN_OPERATIONS_MIN_SELL, MIN_OPERATIONS_MAX_SELL + MIN_OPERATIONS_STEP, MIN_OPERATIONS_STEP):
                                                ...
                                                # get data
                                                # ----
                                                bb_lower, bb_upper, rsi, prices_mean, prices_low, prices_high = mirai_no_data(
                                                ...
                                                slope_buy, intercept_buy, score_buy = mirai_no_yaiba(x_buy, y_buy, a_buy, MIN_OPERATIONS=MIN_OPERATIONS_BUY)
                                                slope_sell, intercept_sell, score_sell = mirai_no_yaiba(x_sell, y_sell, a_sell, MIN_OPERATIONS=MIN_OPERATIONS_SELL)
...
for i in range(0, 588):
    x = threading.Thread(target=mirai_no_sentaku, args=[i+1,588,date(2004,4,26),date(2022,4,1)])
    x.start()
